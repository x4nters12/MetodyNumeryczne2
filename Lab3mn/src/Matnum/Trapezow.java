...
public class Trapezow {
    ...
    private static void setSLICES(){
        System.out.print("Podaj ilość kawałków > ");
    ...
    private static void aproximate(){
        DX = (RIGHT - LEFT) / SLICES; // OBLICZA SZEROKOSC POJEDYNCZEGO TRAPEZU
        RESULT = 0;

        for(int index=1; index<SLICES; index++){
            RESULT += calc(LEFT + index * DX);
        }

        RESULT = DX * (RESULT + ((calc(LEFT) + calc(RIGHT)) / 2));
    }
